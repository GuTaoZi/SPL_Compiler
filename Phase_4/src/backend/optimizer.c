...
bool equal_parent(Parent a, Parent b)
{
    ...
    if (!not_tmp(a.var) || !not_tmp(b.var) || (a.var == NULL) + (b.var == NULL) == 1)
        return false;
    return a.usage == b.usage && ((a.var == b.var && a.recent == b.recent) ||
...
{
    return a == b || (equal_parent(a->parent[0], b->parent[0]) && equal_parent(a->parent[1], b->parent[1])) ||
...
bool simplify_assign(IR_list *ir)
{
    ...
    case 5:
        ...
        // #0
        else if ((op == '*' && ((!not_tmp(y) && y->type == CONST && y->val == 0) ||
                                (!not_tmp(z) && z->type == CONST && z->val == 0))) ||
        ...
        // ? + 0, ? - 0
                 ...
                 ((op == '+' && z->type == CONST && z->val == 0) || (op == '-' && z->type == CONST && z->val == 0)))
        {
            // fprintf(debug, "+- 0\n");
            free(ir->ss[3]);
            free(ir->ss[4]);
...
bool opt_read_func(IR_list *root)
{
    ...
    while (ir != NULL)
    {
        if (strcmp(ir->ss[0], "READ") == 0)
        {
            ...
            if (strcmp(next->ss[1], ":=") == 0 && next->ss[0][0] != '*' && strcmp(ir->ss[1], next->ss[2]) == 0 &&
                next->ss[3] == NULL)
            {
                free(ir->ss[1]);
